...
@onready var _transition_viewport: SubViewport = find_child("transition_viewport")
...
# sets a custom scene as the transition UI (replaces the solid color during transitions)
# the scene content will be revealed through the fade pattern like a mask effect
# pass null to remove and revert to solid color
func set_transition_scene(scene: PackedScene) -> void:
	for child in _transition_viewport.get_children():
		child.queue_free()
	if scene:
		var instance = scene.instantiate()
		_transition_viewport.add_child(instance)

# returns the SubViewport so you can add children directly
func get_transition_viewport() -> SubViewport:
	return _transition_viewport

func _enable_transition_bg() -> void:
	if _transition_viewport.get_child_count() > 0:
		_transition_viewport.size = get_viewport().get_visible_rect().size
		_fade_color_rect.material.set_shader_parameter("background_texture", _transition_viewport.get_texture())
		_fade_color_rect.material.set_shader_parameter("use_background_texture", true)

func _disable_transition_bg() -> void:
	_fade_color_rect.material.set_shader_parameter("use_background_texture", false)
	_fade_color_rect.material.set_shader_parameter("background_texture", null)
